...
This plan should finish the media-to-move workflow before starting migration matching, security, or admin activity tracking. Count overlays are now tracked as an independent optional feature, not as a blocker for the core media-to-move workflow. Items that appear implemented in source but have not been browser-verified are called out as verification tasks rather than assumed-new feature work.
...
| P2-Media-2 | Verify changed-unpublished detection for core clip edits | `MediaEdit-7` | Confirm range, crop, label, key-video, and other core clip edits mark published clips as changed drafts and can be republished cleanly. Count-only edits belong to the independent count-overlay feature. |
| P2-Media-3 | Verify crop save and rendered output behavior | `MediaEdit-13`, `MediaEdit-14` | Server-side rendering now applies `cropRect`. Browser-verify crop drawing, saving, render output, and move-page playback variants. |
...
## Priority 4: Independent Count Overlay Feature

Count overlays should behave like captions: optional, metadata-driven, default-off, and separate from whether a media-derived clip can be drafted, rendered, published, or revisited.

| ID | Todo | Source Plan IDs | Notes |
| --- | --- | --- | --- |
| P2-Count-1 | Finish count placement controls in the media editor | `MediaEdit-15`, `MediaEdit-18`, `MediaEdit-19` | Expose visible controls to start count placement, place the current count, finish placement, and understand the active count. Enter-key placement exists in code, but the workflow needs discoverable UI. |
| P2-Count-2 | Add direct corner controls for count overlay placement | `MediaEdit-20` | Replace any dropdown-only placement flow with four explicit corner buttons: top-left, top-right, bottom-left, bottom-right. |
| P2-Count-3 | Expose count timing preset controls | `MediaEdit-21`, `MediaEdit-22`, `MediaEdit-23` | The preset data exists for On2, On2 all, On1, and On1 all. Add the editor UI and make preset changes rebuild the marker sequence intentionally. |
| P2-Count-4 | Finish clear-flag editing for count overlays | `MediaEdit-24` | Count markers can store a clear flag. Add a clear/hold editing control so overlays can disappear before the next count when needed. |
| P2-Count-5 | Verify count preview and runtime toggle end to end | `MediaEdit-25`, `MediaEdit-26` | Move pages have a runtime count toggle when markers exist, and the editor can preview active markers. Browser-verify save, move-page playback, and default-off behavior without making count overlays part of the rendered video. |
| P2-Count-6 | Keep count-only edits separate from core publish readiness | `MediaEdit-7`, `MediaEdit-15` | Count metadata may have its own dirty/saved state, but unfinished count UI should not prevent ordinary clip editing, rendering, or publish-to-moves from being considered complete. |

## Priority 5: Migration Matching

Do not start this until Priority 1 and Priority 2 are closed. Migration relies on draft, render, publish, status, and replacement behavior being trustworthy. It does not depend on the independent count-overlay feature.
...
- Core clip edits can be drafted, rendered, published, revisited, and republished without relying on count overlays.
- Crop and other core non-range edits reliably create changed-unpublished draft state and can be republished.
...
- Count overlays are tracked as an independent optional feature with their own editor workflow and default-off runtime display.
