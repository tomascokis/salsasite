...
```bash
docker exec -it -w /server/live/frontend -e DATA_DIR=/server/live/data/live salsasite-dev npm run user:create -- --username admin --role admin
docker exec -it -w /server/live/frontend -e DATA_DIR=/server/live/data/live salsasite-dev npm run user:create -- --username viewer --role viewer
```
...
```bash
docker exec -it -w /server/live/frontend -e DATA_DIR=/server/live/data/live salsasite-dev npm run user:password -- --username admin --role admin
```

The `-w /server/live/frontend` working directory is required because the account scripts are frontend package scripts. The explicit `DATA_DIR=/server/live/data/live` must match the running site's live data directory; account scripts fail instead of writing users to another SQLite database.
