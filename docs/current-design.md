...
The progress viewer builds snapshot views over the same layout as the overview page. Overview and progress move rows must show the shared move hover preview after 1 second of mouseover, and on keyboard focus, using the same compact video/name/ID preview treatment as relationship diagram move nodes. Each tracked move shows three colored status dimensions:
