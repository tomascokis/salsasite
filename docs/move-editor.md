...
- The editor must always show `Reset zoom` and `Loop zoom` controls in the main timeline action row beside `Edit moves` or `Add move`; they should be disabled when no zoom window is active.
